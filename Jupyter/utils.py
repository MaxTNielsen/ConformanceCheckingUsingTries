...
def get_occ_dicts(dir_path: str, isC_3PO:bool=False) -> dict:
    """ params:
        ...
        returns:
                nested dict with dataframes containing the results for a given log in a run
        keys:
    ...
    for log in LOGS:
        for log_type in LOG_TYPES:
            ...
            log_dfs[log][log_type]['Conformance cost'] = np.array(exe_times)
            log_dfs[log][log_type]['ExecutionTime'] = np.array(conf_cost)
