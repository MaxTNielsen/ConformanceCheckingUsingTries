...
# MODEL_DIR = os.path.join('..','input\\trie-stream\\Petri-net_models\\M-nets\\')
# LOG_DIR = os.path.join('..','output\\hmmconf\\hmmconf_logs\\M-logs\\M1\\')
# RESULT_DIR = os.path.join('..','results', 'hmmconf','test')
...
def import_net(net_fname):
    ...
    net, init_marking, final_marking = pnml_importer(net_fp)
...
def setup_hmm(rg, EM_params):
    ...
    is_inv = lambda t: t.name is None
    startprob = hmmconf.compute_startprob(rg, node_map, is_inv) #hmmconf.compute_startprob(rg, node_map, n_states, is_inv)
    # add epsilon mass to all states
    startprob += 1e-5
    hmmconf.utils.normalize(startprob) #hmmconf.utils.normalize(startprob, axis=1)
    ...
    confmat = hmmconf.compute_conformance_mat(rg, node_map, obsmap, n_obs, n_states) #confmat = hmmconf.compute_conformance_mat(emitmat)
    ...
    # hmm = hmmconf.HMMConf(startprob, transcube, emitmat, confmat, distmat, 
    #                       int2state, int2obs, n_states, n_obs, params='to', verbose=True, 
    #                       n_jobs=EM_params[N_JOBS], tol=EM_params[TOL], 
    #                       n_iter=EM_params[N_ITER], random_seed=EM_params[RANDOM_SEED_PARAM])
    return hmm
