...
def estimate_conform_params(event_df, state2int, obs2int,
    ...
    # get pseudo counts
    if add_prior:
        def is_inv_rg(t): return t.name is None
...
def run(model_fp: str, data_fp: str, conf_out: str, model_name, log_name):
    ...
    obs2int_df = pd.DataFrame(list(obs2int.items()), columns=[
    ...
    with open(conf_out, 'w') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['caseId', 'conformance', 'execution time'])
        for row in filtered_df.itertuples(index=False):
            ...
            csvwriter.writerow([caseid, result[1], trace_time])
            ...
            if total_events % 10000 == 0:
                ...
                msg = 'Total events: {}, ' \
                    'Number of cases: {}, ' \
                    'Memory: {:.2f}MB'.format(
                        total_events, n_cases, sizetracker)
                print(msg)
    ...
    for row in filtered_df.itertuples(index=False):
        ...
        if total_events % 10000 == 0:
            ...
            msg = msg.format(total_events, n_cases,
                             total_time, avg_time, local_avg)
            print(msg)
    ...
    out_fp = os.path.join(results_dir, 'results-stress-test' +
                          '_'+log_name.replace('.csv', ''))
