...
def create_dataset(traces_dict, label_to_idx):
    ...
    for trace in traces_dict.values():
        ...
        input = one_hot_tensor[0:len(trace), :]
        input = torch.cat((torch.zeros(1, n_activities), input))
        inputs.append(input)
        target = torch.tensor([label_to_idx[trace[i]] for i in range(len(trace))] + [label_to_idx['END']])
...
def preprocess_input(trace: list, label_to_idx: dict) -> float:
    ...
    tensor = torch.cat((torch.zeros(1, n_activities), tensor))
