...
def build_traces_dicts(filename: str.__class__):
    ...
    labels_to_idx['END'] = len(labels_to_idx)
    idx_to_labels[len(labels_to_idx)] = 'END'
...
def set_params():
    ...
    global store
    store['model_params'] = {}
    store['model_params']['input_size'] = len(store['labels_to_idx'])
...
def make_prediction(input: dict) -> float:
    ...
    output_ = output[-2][output_idx].item()
    return output_
