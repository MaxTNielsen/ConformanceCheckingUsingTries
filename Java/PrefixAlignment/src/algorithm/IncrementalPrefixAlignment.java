...
public class IncrementalPrefixAlignment {
    ...
    public Map<Integer, Long> getTraceExecutionTime() {
        return traceExecutionTime;
    }

    private final Map<Integer, Long> traceExecutionTime = new TreeMap<>();

    public IncrementalReplayResult<String, String, Transition, Marking, ? extends PartialAlignment<String, Transition, Marking>> apply(final Petrinet net, Marking initialMarking, Marking finalMarking, final XLog log) {
        setupLabelMap(net);
        ...
        //parameters.setLookBackWindow(1);
    ...
    public IncrementalReplayResult<String, String, Transition, Marking, ? extends PartialAlignment<String, Transition, Marking>> apply(final AcceptingPetriNet accNet, final XLog log) {
        ...
        return apply(net, initialMarking, finalMarking, log);
    }

    public IncrementalReplayResult<String, String, Transition, Marking, PartialAlignment<String, Transition, Marking>> apply(final Petrinet net, final Marking initialMarking, final Marking finalMarking, final XLog log, final IncrementalRevBasedReplayerParametersImpl<Petrinet, String, Transition> parameters) {
        return applyGeneric(net, initialMarking, finalMarking, log, parameters);
    }

    public IncrementalReplayResult<String, String, Transition, Marking, MeasurementAwarePartialAlignment<String, Transition, Marking>> applyMeasurementAware(final Petrinet net, final Marking initialMarking, final Marking finalMarking, final XLog log, final IncrementalRevBasedReplayerParametersImpl<Petrinet, String, Transition> parameters) {
        return applyGeneric(net, initialMarking, finalMarking, log, parameters);
    }

    public <A extends PartialAlignment<String, Transition, Marking>> IncrementalReplayResult<String, String, Transition, Marking, A> applyGeneric(final Petrinet net, final Marking initialMarking, final Marking finalMarking, final XLog log, final IncrementalRevBasedReplayerParametersImpl<Petrinet, String, Transition> parameters) {
        ...
        if (parameters.isExperiment()) {
            ...
            IncrementalReplayer<Petrinet, String, Marking, Transition, String, MeasurementAwarePartialAlignment<String, Transition, Marking>, IncrementalRevBasedReplayerParametersImpl<Petrinet, String, Transition>> replayer = IncrementalReplayer.Factory.construct(initialMarking, finalMarking, store, modelSemantics, parameters, labelsInPN, IncrementalReplayer.Strategy.REVERT_BASED);
            return (IncrementalReplayResult<String, String, Transition, Marking, A>) processXLog(log, net, initialMarking, replayer);
        } else {
            Map<String, PartialAlignment<String, Transition, Marking>> store = new HashMap<>();
            IncrementalReplayer<Petrinet, String, Marking, Transition, String, PartialAlignment<String, Transition, Marking>, IncrementalRevBasedReplayerParametersImpl<Petrinet, String, Transition>> replayer = IncrementalReplayer.Factory.construct(initialMarking, finalMarking, store, modelSemantics, parameters, labelsInPN, IncrementalReplayer.Strategy.REVERT_BASED);
            return (IncrementalReplayResult<String, String, Transition, Marking, A>) processXLog(log, net, initialMarking, replayer);
        }
    }

    private boolean isFeasible(String caseId, List<Move<String, Transition>> moves, List<String> trace, Petrinet net, Marking iMarking) {
        boolean res = true;
        ...
        for (Move<String, Transition> move : moves) {
            if (move.getTransition() != null) {
                ...
                if (!res) {
                    System.out.println("Violation for case " + caseId + ", " + "move " + move.toString() + ", at: " + semantics.getStateAsMarking().toString());
                }
            ...
            if (move.getEventLabel() != null) {
                ...
                if (!res) {
                    System.out.println("Violation for case " + caseId + " on label part. original: " + trace.toString() + ", moves: " + moves.toString());
                }
                i++;
            }
            if (!res) break;
    ...
    private <A extends PartialAlignment<String, Transition, Marking>> IncrementalReplayResult<String, String, Transition, Marking, A> processXLog(XLog log, Petrinet net, Marking iMarking, IncrementalReplayer<Petrinet, String, Marking, Transition, String, A, ? extends IncrementalReplayerParametersImpl<Petrinet, String, Transition>> replayer) {
        ...
        IncrementalReplayResult<String, String, Transition, Marking, A> pluginResult = IncrementalReplayResult.Factory.construct(IncrementalReplayResult.Impl.HASH_MAP);
        /*for (XTrace t : log) {
        ...
        for (XTrace t : log) {
            ...
            for (String e : traceStrLst) {
                partialAlignment = replayer.processEvent(caseId, e);
                pluginResult.get(traceStr).add((A) partialAlignment);
            }
            executionTime = System.currentTimeMillis() - start;
            traceExecutionTime.put(traceId, executionTime);
            if (partialAlignment != null) {
                assert (isFeasible(caseId, partialAlignment, traceStrLst, net, iMarking));
            } else {
                assert (false);
    ...
    public static <A extends PartialAlignment<String, Transition, Marking>> A processEventUsingReplayer(String caseId, String event, IncrementalReplayer<Petrinet, String, Marking, Transition, String, A, ? extends IncrementalReplayerParametersImpl<Petrinet, String, Transition>> replayer) {
