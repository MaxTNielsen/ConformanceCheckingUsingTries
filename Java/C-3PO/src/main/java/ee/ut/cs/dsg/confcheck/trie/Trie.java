...
public class Trie {
    ...
    private void computeConfidenceCostMin(TrieNode root_, boolean isWeighted) {
    ...
    public void computeScaledConfidenceCost(TrieNode root_, boolean isWeighted) {
        for (TrieNode n : root_.getAllChildren()) {
            if (!n.isEndOfTrace()) {
                double x_std = Math.round(((n.getConfidenceCost() - minConf) / (maxConf - minConf)) * 100.0) / 100.0;
                if (isWeighted) {
    ...
    public void computeAndUsePrefProbOnly(TrieNode root_) {
        List<TrieNode> children = root_.getAllChildren();
        /*if (children.size() == 1 && !children.get(0).isEndOfTrace()) {
